...
const LNADDRESS = 'unllamas@blink.sv';

export function useLightning(lnaddress: string) {
  ...
  const handleCreateInvoice = useCallback(
    async (amount: number, currency: string): Promise<any> => {
      ...
      try {
        ...
        const data = await generateLightningInvoice(lnaddress as string, valueInSats);
